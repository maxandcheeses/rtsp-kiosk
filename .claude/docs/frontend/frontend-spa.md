...
The `.views-toolbar` has info icon tooltips (`.info-tooltip` / `.info-icon` / `.tooltip-bubble`) on the Cycle and Default labels. Hovering (or focusing via keyboard) the ℹ icon reveals a `.tooltip-bubble` absolutely positioned above it.

Each static tab panel (`#stab-general`, `#stab-views`, `#stab-cameras`, `#stab-performance`, `#stab-streams`) has a leading `<p>` description element with `color:rgba(255,255,255,0.4);font-size:12px;margin:0 0 20px` as its first (or second, after `<h1>` in General) child. The Actions tab description is injected at the top of `_renderAeTabsInto()` in `actions-editor.js`; subtab descriptions are prepended by each `_buildAe*Tab()` function using a `descHtml` const.
...
`activateSettingsTab` also calls `setSettingsFooter(text)` (defined in `ui.js`) to populate `#settings-tab-footer` — a fixed footer bar at the bottom of `.settings-content` — with the tab's description. `switchAeTab` in `actions-editor.js` overrides it with the active Actions subtab description.
...
|---|---|---|---|---|
| `--fs-xs` | 11px | 0.65vw | 18px | `.chrome .lbl`, `.err-sub` |
| `--fs-sm` | 13px | 0.75vw | 20px | `.chrome .live`, `.err-code` |
| `--fs-md` | 14px | 0.85vw | 24px | (reserved for larger labels) |
...
### Legibility improvements (2026-04-20)

A full Phase 1–3 CSS legibility pass was applied to improve readability under bright kiosk lighting:

- **Font sizes** increased ~30–40%: `--fs-xs` 8→11px min, `--fs-sm` 9→13px min, `--fs-md` 10→14px min
- **Chrome text opacity**: `.chrome .lbl` 0.35→0.75, `.chrome .live` 0.4→0.85; both gain `font-weight: 500`
- **Text-shadow**: multi-layer dark halo added to `.chrome .lbl` and `.chrome .live` for legibility over bright video
- **`.chrome::before`**: gradient backdrop (dark top/bottom, transparent middle) ensures chrome text always has a dark backing
- **Error overlay**: `.err-code` at full opacity `#f87171` with `font-weight: 600`; `.err-sub` boosted from 0.15→0.5 opacity
- **Cycle indicator**: background 0.75→0.92, border thickened to 2px, `box-shadow` added; paused/playing border-color 0.4→0.7 opacity
- **Modal font**: `.modal` base font-family switched to system sans-serif stack; monospace preserved for code/data elements
- **Modal text**: `.modal h1` 13→15px, opacity 0.7→0.85, `font-weight: 500`; `.modal-hint` 11→12px, opacity 0.35→0.55
- **Table text**: `.streams-table` base 13→14px, color 0.75→0.85; `th` 0.4→0.6 opacity, 11→12px
- **Debug overlay**: background 0.72→0.88, border 0.12→0.2; `.dbg-key` 0.4→0.6, `.dbg-val` 0.9→1.0

### Touch-friendly sizing (settings page)

All interactive elements in the settings page target a minimum 44px tap target:

| Element | Size | Notes |
|---------|------|-------|
| `.sp-btn` | 44×44px | Row action buttons (test, edit, delete, activate, clone) — unified size used in both Cameras and Views tabs |
| `.perf-reset` | min-height 44px | Secondary/cancel/reset buttons |
| `.cam-add-btn`, `.cam-save-btn`, `.cam-apply-btn` | min-height 44px | Primary camera action buttons |
| `.views-input`, `.perf-input` | min-height 44px | Form inputs and dropdowns |
| `.settings-tab-btn` | min-height 44px | Sidebar navigation buttons |
| `.ae-tab-btn` | min-height 44px | Actions sub-tab buttons |
| `.toggle` | 46px wide, 26px track + 9px padding top/bottom | Padding extends the tap target to 44px tall |
| `.modal-close-btn` | 44×44px | Modal close button |

Base font size for settings page content: 13px. Section labels/hints: 11px. The Views-tab-specific `#stab-views` overrides were reduced to font-weight only — all sizes now come from the unified base rules.

### Settings tab scroll architecture

The settings modal uses a **flex scroll architecture** rather than sticky-header hacks. `.settings-content` is a flex column container with `overflow:hidden` — it does not scroll. Each `.settings-tab-panel` fills the available height (`flex:1; display:flex; flex-direction:column; min-height:0; overflow:hidden; padding:32px 68px 32px 40px`). The right padding of 68px leaves clear breathing room for the `.modal-close-btn` (`position:absolute; top:10px; right:12px`).
