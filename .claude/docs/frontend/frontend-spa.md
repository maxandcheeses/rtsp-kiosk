...
### Sticky headers in settings tabs

`.settings-content` is the scroll container (`overflow-y: auto; padding: 32px 56px 32px 40px`). Three tab toolbar/header elements are sticky within it:

| Element | Tab | CSS |
|---------|-----|-----|
| `.views-toolbar` | Views | `position:sticky; top:0; z-index:4; background:rgb(13,13,13)` |
| `.cam-toolbar` | Cameras | `position:sticky; top:0; z-index:4; background:rgb(13,13,13)` |
| `.ae-tab-nav` | Actions | `position:sticky; top:0; z-index:2; background:rgb(13,13,13)` |
| General tab `h1` | General | `position:sticky; top:0; z-index:2; background:rgb(13,13,13)` |

`top:0` sticks at the `padding-top` boundary of `.settings-content`, which is the correct visible top edge.

`.streams-table th` uses `position:sticky; top:0; z-index:3; background:rgb(13,13,13)` as the base rule. For the Views and Cameras tabs (which have a sticky toolbar above), scoped overrides set `top:44px` to position the thead row below the toolbar:
- `#stab-views .streams-table th { top: 44px }`
- `#stab-cameras .streams-table th { top: 44px }`

The Streams tab (`#stab-streams`) has no toolbar so its thead sticks at `top:0` (base rule).

**Z-index ladder (within `.settings-content` stacking context):** toolbars (`z-index:4`) > table thead (`z-index:3`) > other sticky headers (`z-index:2`). This ensures toolbars always paint above table headers, and table headers always paint above scrolling tbody rows.
