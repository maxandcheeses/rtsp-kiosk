...
`handleDiscoveryMessage(topic, rawPayload)` extracts the action key from topic path segments between the component and `config` (via `_extractDiscoveryKey`). Empty/null payload removes the entry from `DISCOVERED_ACTIONS`. Valid JSON is parsed and passed to `parseHaPayload(component, haPayload, key, cfg)` which maps HA component types (`button`, `switch`, `light`, `lock`) to kiosk action objects. Unsupported types log a warning and return null. On success, the action is stored in `DISCOVERED_ACTIONS` and both `_refreshActionButtons()` and `renderActionsEditor()` are called.
