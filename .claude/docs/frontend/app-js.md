...
| `actions-editor.js` | `AE_FULL/AE_LOCAL/AE_UNSAVED/AE_TAB/AE_OPEN_DRAWER` globals, `loadActionsEditorData()`, `renderActionsEditor()`, `_renderAeTabsInto()`, `switchAeTab()`. **MQTT tab**: `_buildAeMqttTab()` (CRUD table of `mqtt.servers`), `_buildAeSrvDrawerForm()`, `openAeSrvDrawer()`, `saveAeSrvDrawer()` (cascades id rename to `action.mqttServer`), `deleteAeSrv()`/`confirmDeleteAeSrv()`, `addAeSrv()`, `_initAeSrvDrag()` + handlers. **Actions tab**: `_buildAeActionsTab()` (renders read-only built-in actions section at top using `BUILTIN_ACTIONS`, then user-defined actions with CRUD table), `_buildAeActionDrawerForm()` (type dropdown: `mqtt`/`focus-panel`; MQTT fields wrapped in `#ae-mqtt-fields`; focus-panel timeout in `#ae-focus-fields`; `_aeTypeChanged(type)` shows/hides sections), `saveAeActionDrawer()` (validates `description` field; validates MQTT fields only for type=mqtt; captures timeout for focus-panel), `_initAeActionDrag()` + handlers. **Groups tab**: `_buildAeGroupsTab()`, `_buildAeGroupDrawerForm()` (slot selects include `BUILTIN_ACTIONS` entries), `_aeAddSlot()` (also includes builtins), `saveAeGroupDrawer()`, `_initAeGroupDrag()` + handlers. `markAeUnsaved()`, `applyAeChanges()`, `discardAeChanges()`. |
