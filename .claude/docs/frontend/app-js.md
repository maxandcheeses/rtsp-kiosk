...
| `camera-editor.js` | `CAM_*` globals, `CAM_FIELD_SCHEMA` (field definitions with `section`, `type`, `showIf`, `dependsOn`, `hint`, `tooltip`), `renderCamerasTab()`, `openCamerasModal()` (shim), `loadCamStreams()`, all `cam*` functions, `_camTestPC` global, `openCamStreamTest(path)`, `closeCamStreamTest()`. **`showIf` fields**: fields with a `showIf` predicate are always rendered into the DOM (wrapped in `display:none` when `showIf()` is false) so save logic can always read their values without null-checking. |
...
| `actions-editor.js` | `AE_FULL/AE_LOCAL/AE_UNSAVED/AE_TAB/AE_OPEN_DRAWER/AE_DRAWER_DIRTY` globals, `loadActionsEditorData()` (async; wraps `renderActionsEditor()` call in try/catch to surface render errors), `renderActionsEditor()`, `_renderAeTabsInto()`, `switchAeTab()`. **TLS cert helpers**: `_AE_CERTS_KEY`/`_AE_MQTT_PORT_DEFAULTS` consts; `_aeCertsLoad/Save/List/Get/Add/Delete()` — read/write `localStorage['mqtt_certs']` as a `{ filename: pemString }` JSON map. **Broker URL parser**: `_aeParsesBrokerUrl(broker)` — parses a legacy `ws://host:port` URL into `{ connectionType, host, port }` for backward compat. **MQTT tab**: `_buildAeMqttTab()` (CRUD table of `mqtt.servers`; each row has Connect (`aeConnectServer(id)`) and Disconnect (`aeDisconnectServer(id)`) buttons), `_buildAeSrvDrawerForm()` (structured fields: Connection Type dropdown `ws/wss/mqtt/mqtts` + Host input + Port number input; on type change `aeOnConnTypeChange()` auto-updates port if it matches a known default and shows/hides `#ae-tls-section`; TLS section shows CA/Client Cert/Client Key dropdowns populated from localStorage + file upload widget for cert storage; cert helpers: `_aeBuildCertList()`, `aeUploadCert()`, `aeDeleteCert(name)`, `_aeRefreshCertUI()`), `openAeSrvDrawer()`, `saveAeSrvDrawer()`, `_aeServerStatusHtml(serverId)`, `aeConnectServer(id)`, `aeDisconnectServer(id)`, `deleteAeSrv()`/`confirmDeleteAeSrv()`, `addAeSrv()`, `_initAeSrvDrag()` + `_onAeSrvDrag*` handlers (uses `_aeSrvDrag` state variable — distinct from the `_aeDrag` variable used by Action/Group/Slot drag). **Tab nav layout**: `_renderAeTabsInto()` renders `.ae-tab-nav` (non-sticky flex row) with the active tab's add button pushed right via `margin-left:auto`. Tab body renders in `#ae-tab-content.ae-tab-body` (scroll container). **Actions tab**: `_buildAeActionsTab()` (renders built-in actions section + user-defined actions CRUD table; discovered actions sourced from `DISCOVERED_ACTIONS` global — each row includes `.action-discovered-badge` span), `_buildAeActionDrawerForm()` (focus-stream type shows panel select: "Same Panel" or "Stream 1–8" dropdown, maps to `action.panel` int; `action.panel` missing → defaults to same panel / `_actionsSlotIndex` at runtime), `saveAeActionDrawer()`, `_aeStateHtml(action)` (renders coloured dot + value for MQTT state topics), `_initAeActionDrag()` + `_onAeActionDrag*` handlers (uses `_aeDrag` variable). **Groups tab**: `_buildAeGroupsTab()`, `_buildAeGroupDrawerForm()`, `_aeAddSlot()`, `saveAeGroupDrawer()`, `_initAeGroupDrag()` + `_onAeGroupDrag*` handlers (uses `_aeDrag`), `_initAeSlotDrag()` + `_onAeSlotDrag*` handlers (uses `_aeDrag`). `markAeUnsaved()`, `applyAeChanges()`, `discardAeChanges()`. |
...
`initDiscovery(cfg)` is called from `loadActionsConfig()`. When `cfg.enabled` is false it is a no-op. When enabled, it stores `cfg` in `_discoveryConfig`, calls `getOrCreateMqttClient` for `cfg.mqttServer` if set, and subscribes to both `${prefix}/+/+/config` and `${prefix}/+/+/+/config` via `mqttSubscribe`. Both unsubscribers are stored in `_discoveryUnsubscriber` as a combined function.

`handleDiscoveryMessage(topic, rawPayload)` extracts the action key from topic path segments between the component and `config` (via `_extractDiscoveryKey`). Empty/null payload removes the entry from `DISCOVERED_ACTIONS`; if the removed action had a `state.topic`, its subscription is cleaned up via `_discoveredStateUnsubs` and `ACTION_STATES[topic]` is deleted. Valid JSON is parsed and passed to `parseHaPayload(component, haPayload, key, cfg)` which maps HA component types to kiosk action objects. Supported types: `button` (mqtt, one-shot press), `switch`/`light` (toggle with optional state), `lock` (toggle with lock/unlock payloads), `binary_sensor` (read-only, `publish: null`, state tracks `payload_on`/`payload_off`; icon from `device_class`), `sensor` (read-only, `publish: null`, `state.unit` set from `unit_of_measurement`; icon from `device_class`), `cover` (toggle open/close; garage icon when `device_class === 'garage'`), `fan` (toggle on/off), `scene` (mqtt one-shot, publishes `payload_on` to `command_topic` or `topic`), `select` (mqtt, publishes first option from `options` array). Unsupported types log a warning and return null. On success, the action is stored in `DISCOVERED_ACTIONS`; if it has a `state.topic` and that topic is not already subscribed (guarded by `_discoveredStateUnsubs`), an MQTT subscription is created and its unsubscriber stored in `_discoveredStateUnsubs[stateTopic]`. Both `_refreshActionButtons()` and `renderActionsEditor()` are called. Read-only types (`binary_sensor`, `sensor`) have `publish: null` — `pressAction` silently no-ops at line 298 (`if (!action.publish) return`).

`handleNativeDiscoveryMessage` follows identical subscribe/unsubscribe logic for native kiosk-format discovery payloads.

`_discoveredStateUnsubs` — module-level `{}` map (state topic → unsubscribe fn) tracking subscriptions created for discovered action state topics. Cleared on `loadActionsConfig()` reload alongside `_actionUnsubscribers`. Guards against double-subscription when the same discovery message arrives more than once.
...
`settings-modal` is tracked via `anyOpen` (uses `classList.contains('open')`). The cam-test modal (`cam-test-modal`) and actions modal (`ACTIONS_MODAL_OPEN`) use `display:none`/`display:flex` toggling instead of a CSS class — they are checked separately in the ESC handler before `anyOpen`. `closeAllModals()` calls `closeCamStreamTest()` to tear down any active test WebRTC connection. If a new modal is added, update `closeAllModals()` and add an ESC handler check appropriate to how the modal tracks its open state.
