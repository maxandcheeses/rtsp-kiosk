...
| `camera-editor.js` | `CAM_*` globals, `renderCamerasTab()`, `openCamerasModal()` (shim), `loadCamStreams()`, all `cam*` functions, `_camTestPC` global, `openCamStreamTest(path)`, `closeCamStreamTest()` |
