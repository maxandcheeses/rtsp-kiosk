...
### `getMergedActions()` / `DISCOVERED_ACTIONS` — actions.js
`DISCOVERED_ACTIONS` is a runtime-only map of actions published via MQTT discovery. `getMergedActions()` returns `{ ...DISCOVERED_ACTIONS, ...ACTIONS }` — static and builtin actions always win. All action *lookup* code uses `getMergedActions()` instead of `ACTIONS` directly. `DISCOVERED_ACTIONS` is reset on every `loadActionsConfig()` call.

`initDiscovery(cfg)` subscribes to `cfg.topic + '/+'` via `mqttSubscribe` and stores the unsubscriber in `_discoveryUnsubscriber` (cleared on each call to allow safe re-init). When `cfg.enabled` is false, discovery is a no-op. `handleDiscoveryMessage(topic, payload)` extracts the action name from the last topic segment; null/empty payload removes the entry; valid JSON sets it; static name collision is skipped with a console log. `_refreshDiscoveredActions()` calls `_refreshActionButtons()` if the actions modal is open, and calls `renderActionsEditor()` if `#ae-content` is present.
