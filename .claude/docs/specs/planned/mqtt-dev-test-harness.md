...
- [ ] `docker compose -f docker-compose.dev.yml up` starts Mosquitto alongside the existing services
- [ ] `http://localhost:8080/mqtt-test.html` loads and shows "Connected" within 2s of page load
- [ ] All state topics from `actions.json` appear as toggle buttons on the test page
- [ ] Clicking a toggle publishes to the state topic; the kiosk action button glows green/off accordingly
- [ ] Pressing an action button in the kiosk appears in the test page command log within 1s
- [ ] Manual publish form works for arbitrary topic/payload
- [ ] No authentication required (anonymous connections allowed)
- [ ] Mosquitto ports `1883` and `9001` are exposed on localhost
