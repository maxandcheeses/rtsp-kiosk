...
Retry delay: ${retryDelay[i] || 0}ms`;
  ...
  function _renderVeStreamPicker() {
    const available = STREAMS.map(s => s.path).filter(p => !_editStreams.includes(p));
    let html = '<div class="sp-list">';

    if (_editStreams.length > 0) {
      html += '<div class="sp-section-label">In view (ordered)</div>';
    ...
    if (available.length > 0) {
      html += '<div class="sp-section-label">Available</div>';
  ...
  function _veStreamAdd(path) {
    _editStreams.push(path);
    _renderVeStreamPicker();
    _renderVeLayoutGrid();
  }

  function _veStreamRemove(idx) {
    _editStreams.splice(idx, 1);
    _renderVeStreamPicker();
    _renderVeLayoutGrid();
  }
  ...
  function saveViewForm() {
    ...
    if (_editStreams.length === 0) { alert('Add at least one stream'); return; }
  ...
  function buildCamDrawerForm(stream, isNew) {
    ...
    return `<div class="cam-form-grid">
      ${camFormRow('Path', `<input class="views-input" id="cam-field-path" value="${camEscHtml(stream.path||'')}" ${isNew ? '' : 'disabled style="opacity:0.5"'} placeholder="my-camera">
        ${!isNew ? '<div style="font-size:9px;color:rgba(255,255,255,0.25);margin-top:2px;font-family:\'Courier New\',monospace">Path cannot be changed after creation</div>' : ''}
  ...
  function saveCamDrawer(originalPath, isNew) {
    ...
    const path = isNew ? (pathEl ? pathEl.value.trim() : originalPath) : originalPath;
    if (isNew) {
  ...
  function markCamUnsaved() {
    ...
    if (text) {
      const changed = CAM_LOCAL_STREAMS.filter(s => {
        const orig = (CAM_STREAMS_FULL || []).find(o => o.path === s.path);
  ...
  async function applyCamChanges() {
    ...
    try {
      const res = await fetch('/api/streams', {
        ...
        body: JSON.stringify(CAM_LOCAL_STREAMS),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      CAM_STREAMS_FULL = JSON.parse(JSON.stringify(CAM_LOCAL_STREAMS));
      CAM_UNSAVED = false;
      const banner = document.getElementById('cam-banner');
