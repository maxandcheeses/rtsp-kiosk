...
function _buildAeMqttTab() {
  const m = (AE_LOCAL && AE_LOCAL.mqtt) || {};
  return `
    <div class="cam-form-grid" style="max-width:520px;margin-top:4px">
      <div class="views-form-row">
        <label style="width:120px;flex-shrink:0;font-size:10px;letter-spacing:0.1em;color:rgba(255,255,255,0.4)">Broker URL</label>
        <input class="views-input" id="ae-mqtt-broker" value="${_aeEsc(m.broker)}" placeholder="ws://host:1883/mqtt"
          oninput="AE_LOCAL.mqtt.broker=this.value;markAeUnsaved()">
      </div>
      <div class="views-form-row">
        <label style="width:120px;flex-shrink:0;font-size:10px;letter-spacing:0.1em;color:rgba(255,255,255,0.4)">Username</label>
        <input class="views-input" id="ae-mqtt-username" value="${_aeEsc(m.username)}" placeholder="optional"
          oninput="AE_LOCAL.mqtt.username=this.value;markAeUnsaved()">
      </div>
      <div class="views-form-row">
        <label style="width:120px;flex-shrink:0;font-size:10px;letter-spacing:0.1em;color:rgba(255,255,255,0.4)">Password</label>
        <input class="views-input" type="password" id="ae-mqtt-password" value="${_aeEsc(m.password)}" placeholder="optional"
          oninput="AE_LOCAL.mqtt.password=this.value;markAeUnsaved()">
      </div>
    </div>`;
}
...
function _buildAeActionsTab() {
  ...
  actions.forEach(action => {
    ...
    rows += `<tr id="ae-action-row-${esc}" style="cursor:pointer" onclick="(function(e){if(!e.target.closest('button'))openAeActionDrawer('${esc}')})(event)">
      ...
      <td style="font-family:'Courier New',monospace;font-size:9px;color:rgba(255,255,255,0.4);max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${publishSummary}</td>
      <td>${stateInfo}</td>
  ...
  return addBtn + `<table class="streams-table" style="width:100%">
    <thead><tr>
      <th></th><th>ID</th><th>Label</th><th>Icon</th><th>Publish</th><th>State</th><th></th>
    </tr></thead>
...
function closeAeDrawer() {
  ...
  const drawer = document.getElementById(`ae-action-drawer-${AE_OPEN_DRAWER}`) ||
                 document.getElementById(`ae-group-drawer-${AE_OPEN_DRAWER}`);
  if (drawer) drawer.style.maxHeight = '0';
  const row = document.getElementById(`ae-action-row-${AE_OPEN_DRAWER}`) ||
...
function addAeAction() {
  ...
  const newAction = { id: `new-action-${n}`, label: '', publish: { topic: '', payload: '' } };
